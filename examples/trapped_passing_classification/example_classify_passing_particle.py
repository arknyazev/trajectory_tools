...
tj = Trajectory('example_passing_trajectory_tys.txt')
...
assert particle_type == TrajectoryType.PASSING
...
ax2.plot(tj.t, tj.s, label='Passing',color='r')
...
plt.tight_layout()
plt.savefig(f'pseudo_cartesian_plot_for_passing_particle_example.png', dpi=150)
plt.close()

plt.sca(ax2)
plt.legend(frameon=False)
plt.tight_layout()
plt.savefig(f'timeplot_for_passing_particle_example.png', dpi=150)
